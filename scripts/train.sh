...
echo "task=$task, vit=$vit, dataset=$dataset, shot=$shot, split=$split, num_gpus=$num_gpus, run_date=$run_date"
...
case "$task" in
    fsod)
        if [[ "$dataset" == "coco" ]]; then
            python3 tools/train_net.py \
                ...
                MODEL.WEIGHTS "${weights_root}/initial/few-shot/vit${vit}+rpn.pth" \
                DE.OFFLINE_RPN_CONFIG configs/RPN/mask_rcnn_R_50_C4_1x_fewshot_14.yaml \
                OUTPUT_DIR "output/train/few-shot/shot-${shot}/vit${vit}/${run_date}/" \
                "$@"
        else
            python3 tools/train_net.py \
                --num-gpus "$num_gpus" \
                --config-file "configs/few-shot-voc/${shot}shot/vit${vit}_${split}s.yaml" \
                MODEL.WEIGHTS "${weights_root}/initial/few-shot-voc/voc${split}/${vit}+rpn.pth" \
    ...
    osod)
        python3 tools/train_net.py \
            ...
            MODEL.WEIGHTS "${weights_root}/initial/oneshot/vit${vit}+rpn.split${split}.pth" \
